    ...
    def export_stock_inventory(self):
        ...
        for rec in self:
            ...
            if rec.line_ids:
                ...
                worksheet.write(row, col+5, 'System Quantity', heading)
        ...
        for line in self:
            ...
            if lines_count[key] > 1:
                raise UserError(_("There is already one inventory adjustment line for this product,"
                                  " you should rather modify this one instead of creating a new one. Inventory = %s , product - %s and location - %s and package - %s") %(line.inventory_id.name,line.product_id.default_code, line.location_id.name, line.package_id.name))