...
"""Create Product."""
file_upload = "Anyfreight master supplier list V2.xlsx"
...
def prepare_contact_vals(row_values):
    zip = isinstance(row_values[11], float) and str(int(row_values[11])) or row_values[11]
    vat = (
    ...
    partner_vals = {
        ...
        'ref' : row_values[1],
        "email": row_values[2],
        "street": str(row_values[5]) + str(row_values[6]),
        "street2": str(row_values[7]) + str(row_values[8]),
        "city": row_values[9],
        "zip": zip,
        "phone": row_values[22],
        'tax_exemption' : True if row_values[27] else False,
        'website' : row_values[28],
        "supplier_rank": 1,
        "company_type": "person",
    }
            ...
            if row_values[10]:
                state_id = sock.execute(
                    *credentials,
                    "res.country.state",
                    "search",
                    [("code", "=", row_values[10]), ("country_id", "=", country_id[0])],
                )
                if state_id:
                    partner_vals["state_id"] = state_id[0]
                else:
                    partner_vals["city"] += row_values[9]

    delivery_address = {}

    if row_values[14]:
        delivery_address['street'] = row_values[14]
    if row_values[15]:
        delivery_address['street2'] = str(row_values[15]) + str(row_values[16]) + str(row_values[17])
    if row_values[18]:
        delivery_address['city'] = row_values[18]
    if row_values[20]:
        delivery_address['zip'] = isinstance(row_values[20], float) and str(int(row_values[20])) or row_values[20]
    if row_values[21]:
            ...
            if row_values[19]:
                del_state_id = sock.execute(
                    *credentials,
                    "res.country.state",
                    "search",
                    [("code", "=", row_values[19]), ("country_id", "=", del_country_id[0])],
                )
                if del_state_id:
    ...
    payment_term = False
    data_term_days = isinstance(row_values[31], float) and str(int(row_values[31])) or row_values[31]
    print("22222222222222222222222", data_term_days)
