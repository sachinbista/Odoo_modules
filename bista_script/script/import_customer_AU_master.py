...
"""Create Product."""
file_upload = "AU master supplier list V2.xlsx"
...
def prepare_contact_vals(row_values):
    ...
    partner_vals = {
        ...
        "email": row_values[2],
        "street": row_values[5],
        "street2": str(row_values[6]) + str(row_values[7]) + str(row_values[8]),
        "city": row_values[9],
        "zip": zip,
        "phone": row_values[21],
        'tax_exemption' : True if row_values[28] else False,
        'website' : row_values[30],
        "supplier_rank": 1,
        "company_type": "person",
    }
    ...
    if row_values[13]:
        delivery_address['street'] = row_values[13]
    if row_values[14]:
        delivery_address['street2'] = str(row_values[14]) + str(row_values[15]) + str(row_values[16])
    if row_values[17]:
        delivery_address['city'] = row_values[17]
    if row_values[19]:
        delivery_address['zip'] = isinstance(row_values[19], float) and str(int(row_values[19])) or row_values[19]
    if row_values[20]:
            ...
            if row_values[18]:
                del_state_id = sock.execute(
                    *credentials,
                    "res.country.state",
                    "search",
                    [("code", "=", row_values[18]), ("country_id", "=", del_country_id[0])],
                )
                if del_state_id:
    ...
    payment_term = False
    data_term_days = isinstance(row_values[34], float) and str(int(row_values[34])) or row_values[34]
