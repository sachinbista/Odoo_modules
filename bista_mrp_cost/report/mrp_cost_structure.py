...
class MrpCostStructure(models.AbstractModel):
    ...
    def get_lines(self, productions):
        """"
            Modify Production cost report and include Labor and Overhead cost
        """
        ...
        for product in productions.mapped('product_id'):
            ...
            if Workorders:
                ...
                for mo_id, dummy_wo_id, op_id, wo_name, workcenter_id, user, duration, cost_hour, currency_rate in self.env.cr.fetchall():
                    ...
                    workcenter_ids.append(workcenter_id)
            ...
            for product_id, mo_id, qty, cost, currency_rate in data:
                cost *= currency_rate
            ...
            for mo in mos:
                cost_items = []
                if mo.mrp_labor_cost:
                    cost_items.append({
                        'name': "Labor Cost",
                        'amount': mo.mrp_labor_cost,
                        'total_cost': mo.mrp_labor_cost * mo.product_qty
                    })
                if mo.mrp_overhead_cost:
                    cost_items.append({
                        'name': "Overhead Cost",
                        'amount': mo.mrp_overhead_cost,
                        'total_cost': mo.mrp_overhead_cost * mo.product_qty
                    })

                if not len(cost_items):
                    continue

                if not operation_cost_breakdown.get(mo.name):
                    operation_cost_breakdown[mo.name] = []
                operation_cost_breakdown[mo.name] += cost_items
            ...
            res.append({
                ...
                'operation_cost_breakdown': operation_cost_breakdown
            })
        return res
