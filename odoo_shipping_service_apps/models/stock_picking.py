...
class StockPicking(models.Model):
    ...
    def _get_cover_amount_without_qty_done(self):
        ...
        move_line_ids = picking_move_lines.filtered(lambda ml: 
            float_compare(ml.qty_done, 0.0, precision_rounding=ml.product_uom_id.rounding) > 0 and not ml.result_package_id)

        if not move_line_ids:
            move_line_ids = picking_move_lines.filtered(lambda ml: 
                        float_compare(ml.reserved_uom_qty, 0.0, precision_rounding=ml.product_uom_id.rounding) > 0 and 
        ...
        if not total_weight:
            total_weight = sum([po.reserved_uom_qty * po.product_id.weight for po in move_line_ids]) 
