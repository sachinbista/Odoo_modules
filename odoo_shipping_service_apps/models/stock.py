...
class StockMoveLine(models.Model):
    ...
    #filter packages based on the manual package created on sale order
    ...
    @api.depends('picking_id', 'picking_id.sale_id', 'picking_id.sale_id.quant_package_ids')
    def _compute_related_package_ids(self):
        for line in self:
            if line.picking_id and line.picking_id.sale_id and line.picking_id.sale_id.quant_package_ids:
                package_ids = line.picking_id.sale_id.quant_package_ids.ids
                line.related_package_ids = package_ids
            else:
                line.related_package_ids = self.env['stock.quant.package'].search([])
