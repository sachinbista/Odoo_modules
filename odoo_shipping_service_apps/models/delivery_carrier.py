...
class DeliveryCarrier(models.Model):
    ...
    def wk_get_order_package(self, order):
        ...
        if order.create_package == 'auto':
            ...
            weight = self.env.context.get('order_weight') or result[0]['weight']
            result[0]['weight'] = weight
        else:
            if not order.quant_package_ids:
                raise ValidationError('Kindly create packages first than click on get rate!')
            return map(lambda wk_packaging_id: dict(
                packaging_id=wk_packaging_id.package_type_id.id,
                weight=wk_packaging_id.shipping_weight,
                width=wk_packaging_id.width,
                length=wk_packaging_id.length,
                height=wk_packaging_id.height
            ), order.quant_package_ids)
        return result
