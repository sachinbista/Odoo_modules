...
class AccountMove(models.Model):
    _inherit = ["account.move", "commission.mixin"]
    _name = 'account.move'

    commission_total = fields.Float(
        string="Agent Commission", compute="_compute_commission_total",
        store=True,
    )
    ...
    agent_ids = fields.One2many("account.invoice.line.agent", "invoice_id")
    ...
    goflow_store_id = fields.Many2one('goflow.store', string="Goflow Store")
    ...
    @api.depends("partner_agent_ids", "invoice_line_ids.move_id.agent_ids.agent_id")
    ...
    def _compute_discount_total(self):
        for record in self:
            ...
            for line in record.line_ids:
                record.discount_total += (line.price_unit * line.quantity) * line.discount / 100
...
class AccountMoveLine(models.Model):
    ...
    def _compute_any_settled(self):
        for record in self:
            record.any_settled = any(record.move_id.mapped("agent_ids.settled"))
    ...
    #         ):
    #             record.agent_ids = record._prepare_agents_vals_partner(
    #                 record.move_id.partner_id, record, settlement_type="sale_invoice"
    #             )
...
class AccountInvoiceLineAgent(models.Model):
    ...
    # object_id = fields.Many2one(comodel_name="account.move")
    ...
    rate = fields.Float(related="commission_id.fix_qty",
                string="Fixed Percentage", store=True)

    @api.depends(
        "invoice_id.amount_total",
        "invoice_id.commission_free",
        "commission_id",
    )
    def _compute_amount(self):
        for agent_line in self:

            invoice_id = agent_line.invoice_id
            ...
            if invoice_id.goflow_store_id and invoice_id.goflow_store_id.id in amazon_store.ids:
            ...
            freight_charge_lines = invoice_id.invoice_line_ids.filtered(
            ...
            # Calculation for the agent's commission for invoices.
            agent_line.amount = agent_line._get_commission_amount(agent_line.commission_id, net_amount)

            # Refunds commissions are negative
            if agent_line.invoice_id.move_type and "refund" in agent_line.invoice_id.move_type:
                agent_line.amount = -agent_line.amount
    ...
    @api.depends("invoice_id", "invoice_id.company_id")
    def _compute_company(self):
        for line in self:
            line.company_id = line.invoice_id.company_id
