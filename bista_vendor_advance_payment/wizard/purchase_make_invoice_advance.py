...
class PurchaseAdvancePaymentInv(models.TransientModel):
    ...
    def default_get(self, fields):
        res = super(PurchaseAdvancePaymentInv, self).default_get(fields)
        order_id = self.env['purchase.order'].browse(
            self._context.get('active_id', []))
        if order_id.invoice_status in ('no', 'invoiced') and order_id.receipt_status in ('pending', 'partial'):
            res.update({'advance_payment_method': 'percentage'})
        return res

    def _get_prepay_bal(self):
        purchase_order_id = self.env['purchase.order'].browse(
            self._context.get('active_ids', []))
        po_prepay_lines_lst = [pol.id for pol in purchase_order_id.order_line.filtered(lambda x: x.is_prepayment)]
        if purchase_order_id.invoice_ids:
            invoice_ids = purchase_order_id.invoice_ids
            invoice_ids = invoice_ids.filtered(lambda x: x.state != 'cancel')
            inv_line_lst = [line.price_subtotal for line in invoice_ids.invoice_line_ids.filtered(
                lambda x: x.purchase_line_id.id in po_prepay_lines_lst)]

            if inv_line_lst:
                total_amount = sum(inv_line_lst)
                return total_amount
        else:
            return 0
    ...
    product_id = fields.Many2one('product.product', string='Pre Payment Product', domain=[('type', '=', 'service')],
                                 default=_default_product_id)
    count = fields.Integer(default=_count, string='Order Count')
    amount = fields.Float('Pre Payment Amount', digits='Account',
                          help="The percentage amount to be invoiced in advance,taxes excluded.")
    ...
    prepay_bal = fields.Float(string="Pre Payment Balance", default=_get_prepay_bal, readonly=True)
    ...
    def create_invoices(self):
        ...
        for order in purchase_orders:
            if order.receipt_status == 'full' and order.invoice_status == 'invoiced':
                raise UserError("Invoice is fully created.")
        paid_prepay_amt = 0
        po_prepay_lines_lst = [pol.id for pol in purchase_orders.order_line.filtered(lambda x: x.is_prepayment)]
        amount_total = purchase_orders.amount_total
        if purchase_orders.invoice_ids:
            invoice_ids = purchase_orders.invoice_ids
            invoice_ids = invoice_ids.filtered(lambda x: x.state != 'cancel' and len(x.invoice_line_ids) == 1)
            inv_line_lst = [line.price_subtotal for line in invoice_ids.invoice_line_ids.filtered(
                lambda x: x.purchase_line_id.id in po_prepay_lines_lst)]
            if inv_line_lst:
                paid_prepay_amt = sum(inv_line_lst)
        create_invoice_ids = []
        if self.advance_payment_method == 'percentage':
            if self.amount <= 0.0:
                raise UserError(_('The percentage should be above zero.'))
            if self.amount > 100.0:
                raise UserError(_('The percentage should be below 100.'))
            if amount_total <= 0:
                raise UserError(_("The amount/percentage can't be greater then order amount."))

            if paid_prepay_amt > 0:
                remain_prepay_percent = abs(round((100 - ((paid_prepay_amt * 100) / amount_total)), 2))
                if self.amount > remain_prepay_percent:
                    raise UserError(f"The Remaining amount is upto {remain_prepay_percent} % \nPlease pay upto it.")

        if self.advance_payment_method == 'fixed':
            if self.fixed_amount == 0:
                raise UserError(_("Amount should be above zero."))
            if self.fixed_amount > purchase_orders.amount_total:
                raise UserError(_("Amount can't be greater then order amount."))
            if paid_prepay_amt:
                remain_prepay_amt = abs(round((amount_total - paid_prepay_amt), 2))
                if self.fixed_amount > remain_prepay_amt:
                    raise UserError(f"The Remaining amount is {remain_prepay_amt} \nPlease pay upto it.")
