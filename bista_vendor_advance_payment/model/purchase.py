...
class PurchaseOrder(models.Model):
    ...
    def _prepare_invoice(self):
        ...
        partner_invoice_id = self.env['res.partner'].browse(self.partner_id.address_get(['invoice'])['invoice'])
...
#         action = self.env.ref('account.action_move_in_invoice_type').read()[0]
        ...
        if len(invoices) > 1:
            action['domain'] = [('id', 'in', invoices.ids), ('move_type', 'in', ['in_invoice', 'in_refund'])]
        elif len(invoices) == 1:
