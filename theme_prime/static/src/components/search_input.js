...
export class TpCardGrid extends AbstractComponent {
    ...
    get nodeOptions() {
        ...
        return {
            style: {
                ...buttonClasses, title: this.env._t("Style"), records: _.map(this.styles, (style, index) => {
                    return {id: style, title: this.env._t(`Style - ${index + 1}`)};
                })
            },
            productListing: {
                ...buttonClasses,
                title: this.env._t("Product Listing"),
                records: [{
                    id: 'bestseller',
                    title: this.env._t("Best Seller"),
                    iconTemplate: 'dri dri-bolt'
                }, {iconTemplate: 'fa fa-percent', id: 'discount', title: this.env._t("Discount")}]
            },
            limit: {title: this.env._t("No. of items"), maxValue: 20, minValue: 0},
            child: {title: this.env._t("No. of Child"), maxValue: 20, minValue: 3},
            brand: {title: this.env._t("Brands")},
            label: {title: this.env._t("Label")},
            count: {title: this.env._t("Count")},
            background: {title: this.env._t("Bg. Image")}
        };
    }
...
export class TpComponentGroup extends AbstractComponent {
    ...
    get nodeOptions() {
        ...
        return {
            style: {
                ...buttonClasses,
                title: this.env._t("Mobile Style"),
                records: _.union([{
                    id: 'default',
                    title: this.env._t('Same as Desktop')
                }], _.map(this.mobileStyles, (style, index) => {
                    return {id: style, title: this.env._t(`Style - ${index + 1}`)};
                }))
            },
            mode: {
                ...buttonClasses,
                title: this.env._t("Mode"),
                records: [{id: 'default', title: this.env._t('Same as Desktop')}, {
                    id: 'grid',
                    iconTemplate: 'fa fa-th-large pe-2',
                    title: this.env._t('Grid')
                }, {iconTemplate: 'fa pe-2 fa-arrows-h', id: 'slider', title: this.env._t('Slider')}]
            },
        };
    }
...
export class TpActions extends Component {
    ...
    _getAction(action) {
        let allActions = {
            colors: {icon: 'theme_prime.icon_brush', label: this.env._t('Colors')},
            count: {icon: 'theme_prime.icon_hash_tag', label: this.env._t("Count")},
            brand: {icon: 'theme_prime.icon_tag', label: this.env._t("Brands")},
            quick_view: {icon: 'theme_prime.icon_eye', label: this.env._t('QUICK VIEW')},
            add_to_cart: {icon: 'theme_prime.icon_cart', label: this.env._t('ADD TO CART')},
            category_info: {icon: 'theme_prime.icon_font', label: this.env._t('CATEGORY')},
            wishlist: {icon: 'theme_prime.icon_heart', label: this.env._t('WISHLIST')},
            comparison: {icon: 'theme_prime.icon_exchange', label: this.env._t('COMPARE')},
            rating: {icon: 'theme_prime.icon_star', label: this.env._t('RATING')},
            description_sale: {icon: 'theme_prime.icon_description', label: this.env._t('DESCRIPTION')},
            label: {icon: 'theme_prime.icon_tag', label: this.env._t('LABEL')},
            show_similar: {icon: 'theme_prime.icon_box', label: this.env._t('SIMILAR')}
        };
...
export class TpDomainComponent extends AbstractComponent {
    ...
    get fieldsList() {
        return [{'type': 'text', 'name': 'name', 'label': this.env._t('Name')}, {
            'type': 'many2many',
            'name': 'public_categ_ids',
            'label': this.env._t('Category'),
            'relationModel': 'product.public.category'
        }, {
            'type': 'many2one',
            'name': 'dr_brand_value_id',
            'label': this.env._t('Brand'),
            'relationModel': 'product.attribute.value',
            'extras': {'brands': true}
        }, {
            'type': 'many2one',
            'name': 'attribute_line_ids.value_ids',
            'label': this.env._t('Attributes'),
            'relationModel': 'product.attribute.value'
        }, {
            'type': 'many2one',
            'name': 'dr_label_id',
        ...
        }, {'type': 'integer', 'name': 'list_price', 'label': this.env._t('Price')}, {
            'type': 'many2one',
            'name': 'product_tag_ids',
            'label': this.env._t('Tags'),
            'relationModel': 'product.tag',
            'is_multi_website': true
        }, {'type': 'boolean', 'name': 'dr_has_discount', 'label': this.env._t('Discount')}];
    }
    ...
    get domainTemplates() {
        return [
            {
                id: 0,
                subtitle: this.env._t("Select any template and modify as per your need."),
                title: this.env._t("Choose Template"),
                domain: [], sortBy: 'create_date desc'
            }, {
                id: 1,
                subtitle: this.env._t("Show newly arrived products based on creation date"),
                title: this.env._t("New Arrival"),
                domain: [],
                sortBy: 'create_date desc'
            }, {
                id: 2,
                subtitle: this.env._t("Show newly arrived products from selected categories"),
                title: this.env._t("Category New Arrival"),
                domain: [["public_categ_ids", "in", []]],
                sortBy: 'create_date desc'
            }, {
                id: 3,
                subtitle: this.env._t("Show newly arrived products from selected brands"),
                title: this.env._t("Brand New Arrival"),
                domain: [["dr_brand_value_id", "in", []]],
                sortBy: 'create_date desc'
            }, {
                id: 4,
                subtitle: this.env._t("Show newly arrived products from selected tags"),
                title: this.env._t("Tags New Arrival"),
                domain: [["product_tag_ids", "in", []]],
                sortBy: 'create_date desc'
            }, {
                id: 5,
                subtitle: this.env._t("Show newly arrived products from selected label."),
                title: this.env._t("Label New Arrival"),
                domain: [["dr_label_id", "in", []]],
                sortBy: 'create_date desc'
            }, {
                id: 6,
                subtitle: this.env._t("Show discounted products based on product pricelist"),
                title: this.env._t("Discounted Products"),
                domain: [['dr_has_discount', '!=', false]],
                sortBy: 'list_price asc'
            }, {
                id: 7,
                subtitle: this.env._t("Show discounted products based on product pricelist from selected categories"),
                title: this.env._t("Category Discounted Products"),
                domain: ['&', ['dr_has_discount', '!=', false], ["public_categ_ids", "in", []]],
                sortBy: 'list_price asc'
            }, {
                id: 8,
                subtitle: this.env._t("Show discounted products based on product pricelist from selected brands."),
                title: this.env._t("Brand Discounted Products"),
                domain: ['&', ['dr_has_discount', '!=', false], ["dr_brand_value_id", "in", []]],
                sortBy: 'list_price asc'
            }, {
                id: 9,
                subtitle: this.env._t("Show discounted products based on product pricelist from selected tags"),
                title: this.env._t("Tags Discounted Products"),
                domain: ['&', ['dr_has_discount', '!=', false], ["product_tag_ids", "in", []]],
                sortBy: 'list_price asc'
            }, {
                id: 10,
                subtitle: this.env._t("Show discounted products based on product pricelist from selected label"),
                title: this.env._t("Label Discounted Products"),
                domain: ['&', ['dr_has_discount', '!=', false], ["dr_label_id", "in", []]],
                sortBy: 'list_price asc'
            }, {
                id: 11,
                subtitle: this.env._t("Show best seller products based on last 30 days sales"),
                title: this.env._t("Best Seller"),
                domain: [],
                sortBy: 'bestseller'
            }, {
                id: 12,
                subtitle: this.env._t("Show best seller products based on last 30 days sales from selected categories."),
                title: this.env._t("Category Best Seller"),
                domain: [["public_categ_ids", "in", []]],
                sortBy: 'bestseller'
            }, {
                id: 13,
                subtitle: this.env._t("Show best seller products based on last 30 days sales from selected brands."),
                title: this.env._t("Brand Best Seller"),
                domain: [["dr_brand_value_id", "in", []]],
                sortBy: 'bestseller'
            }, {
                id: 14,
                subtitle: this.env._t("Show best seller products based on last 30 days sales from selected tags."),
                title: this.env._t("Tags Best Seller"),
                domain: [["product_tag_ids", "in", []]],
                sortBy: 'bestseller'
            }, {
                id: 15,
                subtitle: this.env._t("Show best seller products based on last 30 days sales from selected label."),
                title: this.env._t("Label Best Seller"),
                domain: [["dr_label_id", "in", []]],
                sortBy: 'bestseller'
            }, {
                id: 16,
                subtitle: this.env._t("Show best seller products with discount"),
                title: this.env._t("Discounted Best Seller"),
                domain: [['dr_has_discount', '!=', false]],
                sortBy: 'bestseller'
            }, {
                id: 17,
                subtitle: this.env._t("Show best seller products with discount from the selected categories"),
                title: this.env._t("Category Discounted Best Seller"),
                domain: ['&', ["public_categ_ids", "in", []], ['dr_has_discount', '!=', false]],
                sortBy: 'bestseller'
            }, {
                id: 18,
                subtitle: this.env._t("Show best seller products with discount from the selected brands"),
                title: this.env._t("Brand Discounted Best Seller"),
                domain: ['&', ["dr_brand_value_id", "in", []], ['dr_has_discount', '!=', false]],
                sortBy: 'bestseller'
            }, {
                id: 19,
                subtitle: this.env._t("Show best seller products with discount from the selected tags"),
                title: this.env._t("Tags Discounted Best Seller"),
                domain: ['&', ["product_tag_ids", "in", []], ['dr_has_discount', '!=', false]],
                sortBy: 'bestseller'
            }, {
                id: 20,
                subtitle: this.env._t("Show best seller products with discount from the selected label"),
                title: this.env._t("Label Discounted Best Seller"),
                domain: ['&', ["dr_label_id", "in", []], ['dr_has_discount', '!=', false]],
                sortBy: 'bestseller'
            }, {
                id: 21,
                subtitle: this.env._t("Recently Purchased"),
                title: this.env._t("Recently Purchased"),
                domain: [], sortBy: 'create_date desc'
            }];
    }
    ...
    _prepareRecords(index) {
        ...
        return {
            ...
            extras: {
                ...
                mode: 'badge',
                isBrand: this.getRecord(this.fieldsList, field, 'name').relationModel === 'product.attribute.value' && field === 'dr_brand_value_id' ? true : false
            }
        }
    }
    ...
    get operatorInfo() {
        return {
            ...
            'many2many': {
                value: {'in': "is having", 'not in': "is not having", 'child_of': "is having child"},
                defaultVal: []
            },
            'many2one': {value: {'in': "is having", 'not in': "is not having"}, defaultVal: []},
            'integer': {
                value: {'=': "equals to", '!=': "not equals to", '>': 'greater than', '<': 'less then'},
                defaultVal: 100
            },
            'boolean': {value: {'!=': 'having', '=': 'not having'}, defaultVal: false}
        };
    }
    ...
    get nodeOptions() {
        return {
            limit: {title: this.env._t("No. of items"), maxValue: 20, minValue: 4},
            order: {
                buttonClasses: "btn d-flex btn-sm justify-content-between align-items-center btn-default bg-white border shadow-sm fw-light w-100",
                title: this.env._t("Order By"),
                records: [{id: 'list_price asc', title: this.env._t("Price: Low to High")}, {
                    id: 'list_price desc',
                    title: this.env._t("Price: High to Low")
                }, {id: 'name asc', title: this.env._t("Name: A to Z")}, {
                    id: 'name desc',
                    title: this.env._t("Name: Z to A")
                }, {id: 'create_date desc', title: this.env._t("Newly Arrived")}, {
                    id: 'bestseller',
                    title: this.env._t("Bestseller")
