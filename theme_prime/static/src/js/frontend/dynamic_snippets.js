...
publicWidget.registry.s_tp_mega_menu_category_snippet = RootWidget.extend({
    ...
    _getOptions: function () {
        // BISTA CUSTOM START
        var result = {};
        let fetch_from_menu = this.$el.data('fetch-from-menu');
        var $megaMenu = this.$el.closest('.o_mega_menu');
        // website.menu.category object
        var menu_website_categ_ids = $megaMenu.data('website-menu-category-ids');
        if (fetch_from_menu && menu_website_categ_ids) {
            var categoryPosition = this.selectionInfo.categoryPosition;
            categoryPosition = categoryPosition ? categoryPosition : -1;
            var categoryID = menu_website_categ_ids[categoryPosition];
            if (this.selectionInfo && categoryID) {
                result['menu_website_categ_ids'] = [categoryID];
            }

            var public_categ_ids = $megaMenu.data('public-category-refs');
            var publicCategID = public_categ_ids[categoryPosition];
            if (this.selectionInfo && publicCategID) {
                this.selectionInfo.recordsIDs = [publicCategID];
            }
        }
        // TODO: need to comment out this code later
        // if (fetch_from_menu) {

        //     var public_categ_ids = $megaMenu.data('public-category-refs');
        //     if (this.selectionInfo && public_categ_ids) {
        //         this.selectionInfo.recordsIDs = [public_categ_ids];
        //     }

        // }
        // TODO: END
        if (!('categoryIDs' in result) && this.selectionInfo && this.selectionInfo.recordsIDs) {
            result['categoryIDs'] = this.selectionInfo.recordsIDs;
        }
...
// BISTA_CUSTOM: attribute snippet
publicWidget.registry.s_tp_mega_menu_attribute_snippet = RootWidget.extend({
    selector: '.s_tp_mega_menu_attribute_snippet',
    bodySelector: '.s_tp_mega_menu_attribute_snippet_wrapper',
    bodyTemplate: 's_tp_hierarchical_attribute_wrapper', // TODO: need to change
    controllerRoute:  '/theme_prime_custom/get_megamenu_attribute', // TODO: need to change
    snippetNodeAttrs: (RootWidget.prototype.snippetNodeAttrs || []).concat(['data-selection-info', 'data-ui-config-info']),
    /**
     * @private
     */
    ...
    _getOptions: function () {
        ...
        // website.menu.category object
        var menu_website_attrib_ids = $megaMenu.data('website-menu-attribute-ids');
        if (fetch_from_menu && menu_website_attrib_ids) {
            var attributePosition = this.selectionInfo.attributePosition;
            attributePosition = attributePosition ? attributePosition : -1;
            var menu_product_attribute = menu_website_attrib_ids[attributePosition];
            if (this.selectionInfo && menu_product_attribute) {
                result['menu_website_attrib_ids'] = [menu_product_attribute];
            }
            var attrib_ids = $megaMenu.data('product-attribute-refs');
            if (attrib_ids) {
                var attribID = attrib_ids[attributePosition];
                if (this.selectionInfo && attribID) {
                    this.selectionInfo.recordsIDs = [attribID];
                }
            }

        }
        // TODO: need to comment out this code later
        var related_category = this.$el.data('related-category');
        var public_categ_id = related_category;
        if (fetch_from_menu) {
            var public_categ_id = $megaMenu.data('main-category-ref');
        }
        if (this.selectionInfo && public_categ_id) {
            this.selectionInfo.related_category = public_categ_id;
            result['related_category'] = public_categ_id;
        }
        // TODO: END
        // var product_attribute_array = $megaMenu.data('product-attribute-ref'); // array
        // if (fetch_from_menu && product_attribute_array) {
        //     var attributePosition = this.selectionInfo.attributePosition;
        //     attributePosition = attributePosition ? attributePosition : -1;
        //     var product_attribute = product_attribute_array[attributePosition];
        //     if (this.selectionInfo && product_attribute) {
        //         this.selectionInfo.recordsIDs = [product_attribute];
        //         result['attributeIDs'] = [product_attribute];
        //     }
        // }
        if (!('attributeIDs' in result) && this.selectionInfo && this.selectionInfo.recordsIDs) {
            result['attributeIDs'] = this.selectionInfo.recordsIDs;
        }

        if (this.selectionInfo && this.selectionInfo.recordsIDs) {
            // if (this.selectionInfo.related_category) {
            //     return { attributeIDs: this.selectionInfo.recordsIDs, related_category: this.selectionInfo.related_category };
            // }
            // return { attributeIDs: this.selectionInfo.recordsIDs};
            return result;
        }
        return false;
    },
    ...
    */
    ...
    _processData: function (data) {
        ...
        this.recordsIDs = [];
        _.each(this.selectionInfo.recordsIDs, recordsID => {
            let attributeRec = _.find(data, function (attribute) { return attribute.attrib.id === recordsID; });
            if (attributeRec) {
                result.push(attributeRec);
                this.recordsIDs.push(recordsID);
            }
        });
        return result;
