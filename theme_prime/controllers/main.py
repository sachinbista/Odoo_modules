...
class ThemePrimeWebsiteSale(WebsiteSale):
    ...
    def shop(self, page=0, category=None, search='', min_price=0.0, max_price=0.0, ppg=False, **post):
        post_brand_family = post.get('bf', False)
        ...
        if theme_id and theme_id.name.startswith('theme_prime'):
            ...
            selected_brand_ids = False
            selected_family_list = False
            bfamily_list = False
            if attrib_values:
                attributes_values = [v[1] for v in attrib_values]
                attributes_values = request.env['product.attribute.value'].sudo().browse(attributes_values)
                selected_brand_ids = attributes_values.filtered(lambda attr_value: attr_value.brand_id).mapped('brand_id')
                # Get selected brand family
                bfamily_list = request_args.getlist('brand_family')
                bfamily_list = [eval(bf) for bf in bfamily_list]

            bf_list = []
            if post.get('bf', False):
                brand_family_post_get= request_args.get('bf')
                brand_family_post_get = brand_family_post_get.split(',')
                bf_list_list = [eval(bf) for bf in brand_family_post_get]
                bf_list.append(bf_list_list)
                for a in bf_list_list:
                    bfamily_list.append(a)

            # Filter products based on brand family
            dashed_list_bf = False
            if bfamily_list:
                res_products = response.qcontext.get('products')
                products_brand_family = res_products.filtered(lambda res_prod: res_prod.product_brand_family_id and res_prod.product_brand_family_id.id in bfamily_list)
                response.qcontext.update(products=products_brand_family)
                dashed_list_bf = ','.join([str(elem) for elem in bfamily_list])
            ...
            response.qcontext.update(
                ...
                selected_family_list=bfamily_list if bfamily_list else False,
                dashed_list_bf=dashed_list_bf
            )
        return response
    ...
    def _prepare_filters_domain(self, search=None, pricelist=None, category=None, attrib_values=None, min_price=None, max_price=None, **post):
        options = self._get_search_options(category=category, brand_id=2, attrib_values=attrib_values, pricelist=pricelist, min_price=min_price, max_price=max_price, **post)
...
class ThemePrimeMainClass(http.Controller):
    ...
    @http.route('/theme_prime_custom/get_megamenu_attribute', type='json', auth='public', website=True)
    def get_megamenu_attributes(self, options={}, limit=5, fields=['name', 'id'], order='count', **kwargs):
        menu_website_attrib_ids = options.get('menu_website_attrib_ids', [])
        # If website menu attribute website provide then attribute will be taken from that
        menu_website_attrib_obj = False
        if menu_website_attrib_ids:
            menu_website_attrib_obj = request.env['website.menu.attribute'].browse(int(menu_website_attrib_ids[0])).exists()
            attributes = menu_website_attrib_obj.related_attribute_id
        else:
            attributes = request.env['product.attribute'].browse(options.get('attributeIDs', [])).exists()

        all_attribs = []
        all_attrib_count = {}
        category_obj = request.env['product.public.category'].browse(int(options.get('related_category',-1))).exists()
        options['category_obj'] = category_obj
        if not options.get('skip_attrib_count', False):
            all_attrib_count = request.env['product.template']._get_product_attrib_count(attrib_values=[], domain=request.env['website'].sale_product_domain())
        for attrib in attributes:
            data = {
                'id': attrib.id, 
                'name': attrib.name,
                'count': all_attrib_count.get(attrib['id'], 0),

                # 'website_url': '/shop/attrib/%s' % (slug(attrib)),
                # 'image_url': '/web/image?model=product.attribute&id=%d&field=dr_attribute_icon' % (attrib.id),
            }
            related_menu_obj = False
            if menu_website_attrib_obj:
                related_menu_obj = menu_website_attrib_obj.filtered(lambda c: c.related_attribute_id.id == attrib.id)
            if category_obj:
                data['website_url'] = '/shop/category/%s?attribute=%s' % (slug(category_obj), attrib.id)
            else:
                data['website_url'] = '/shop?attribute=%s' % (attrib.id)
            data['menu_name'] = related_menu_obj.menu_name if related_menu_obj else attrib.name
            data['view_all_name'] = related_menu_obj.view_all_name if related_menu_obj else ''
            all_attribs.append(data)

        return [self._get_megamenu_child_attributes(attrib_id, limit, all_attribs, order, options) for attrib_id in all_attribs]
    
    def _get_megamenu_child_attributes(self, parent_attrib, limit, all_attribs, order, options):
        pav_obj = request.env['product.attribute.value'].search_read(
            domain=[('attribute_id', '=', parent_attrib.get('id')), ('is_published', '=', True)],
            fields=['id', 'attribute_id', 'name'],
            limit=limit,
            order=order
            )
        category_obj = options.get('category_obj', False)
        for pav in pav_obj:
            if category_obj:
                pav['website_url'] = '/shop/category/%s?attrib=%s-%s' % (slug(category_obj), parent_attrib['id'], pav['id'])
            else:
                pav['website_url'] = '/shop?attrib=%s-%s' % (parent_attrib['id'], pav['id'])
            pav['image_url'] = '/web/image?model=product.attribute.value&id=%d&field=dr_image' % (pav['id'])
        return {'attrib': parent_attrib, 'child': pav_obj}
        
        # if order == 'count' or not order:
        #     child_attribs = sorted(child_attribs, key=lambda attrib: attrib.get('count', 0), reverse=True)
        # child_attribs = child_attribs[:limit]
        # remain_limit = limit - len(child_attribs)

        # if remain_limit <= 0 or options.get('onlyDirectChild', False):
        #     return {'attrib': parent_attrib, 'child': child_attribs}
        # for child_attrib in child_attribs[:]:
        #     new_born_child = self._get_megamenu_child_attributes(child_attrib, remain_limit, all_attribs, order, options).get('child')
        #     child_attribs.extend(new_born_child)
        #     remain_limit = limit - len(child_attribs)
        #     if remain_limit <= 0:
        #         break
        # child_pav = pav_obj.read(['id', 'name', 'attribute_id', 'dr_attribute_icon'])
        # return {'attrib': parent_attrib, 'child': child_attribs}
    ...
    def get_megamenu_categories(self, options={}, limit=5, fields=['name', 'id'], order='count', **kwargs):
        menu_website_categ_ids = options.get('menu_website_categ_ids', [])
        # If website menu category website provide then category will be taken from that
        menu_website_categ_obj = False
        if menu_website_categ_ids:
            menu_website_categ_obj = request.env['website.menu.category'].browse(int(menu_website_categ_ids[0])).exists()
            category_ids = menu_website_categ_obj.related_category_id.ids
        else:
        ...
        for category in categories:
            data = {
                ...
                'website_url': '/shop/category/' + slug(category),
            ...
            related_menu_obj = False
            if menu_website_categ_obj:
                related_menu_obj = menu_website_categ_obj.filtered(lambda c: c.related_category_id.id == category.id)
            data['menu_name'] = related_menu_obj.menu_name if related_menu_obj else category.name
            data['view_all_name'] = related_menu_obj.view_all_name if related_menu_obj else ''
...
class ThemeWebsite(Website):
    ...
    def dr_search(self, term, max_nb_chars, options, **kw):
        ...
        search_types = ['products', 'categories', 'autocomplete', 'suggestions', 'brands', 'brand_family']
        ...
        brands_result = False
        brand_family_result = False
        if search_config.get('search_product_brand'):
            brands_result = self.autocomplete(search_type='search_product_brand', term=term,order='name', limit=5, options=options)
            brand_family_result = self.autocomplete(search_type='search_brand_family', term=term, order='name', limit=5, options=options)
        if brands_result:
            results['brands'] = {'results': brands_result['results'],'results_count': len(brands_result['results']), 'parts': {}}
        else:
            results['brands'] = {'results': '', 'results_count': 0,'parts': {}}
        if brand_family_result:
            for bf_result in brand_family_result['results']:
                bf_result['website_url'] = str(bf_result['website_url']).replace('&amp;','&')
            results['brand_family'] = {'results': brand_family_result['results'], 'results_count': len(brand_family_result['results']),'parts': {}}
        else:
            results['brand_family'] = {'results': '', 'results_count': 0, 'parts': {}}
