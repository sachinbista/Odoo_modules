...
class ShipmentDataQueue(models.Model):
    _name = "shipment.data.queue"
    _description = 'Shipment Data Queue'
        ...
        for vals in vals_list:
            if not vals.get('name'):
                vals['name'] = self.env['ir.sequence'].next_by_code(
                    'shipment.data.queue') or _('New')
        return super(ShipmentDataQueue, self).create(vals_list)

    def export_data(self):
        """
            Button action to export the single shipment data queue to outbound file path
            :return:
            @author: Gauri Shenoy @Bista Solutions Pvt. Ltd.
        """
        for rec in self:
