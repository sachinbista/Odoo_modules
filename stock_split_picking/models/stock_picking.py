...
class StockPicking(models.Model):
    ...
    def split_process(self):
        ...
        for picking in self:
            ...
            if all([x.quantity == 0.0 for x in picking.move_line_ids]):
                raise UserError(
                    _(
                ...
                if qty_diff_compare < 0:
                    ...
                    for move_line in move.move_line_ids:
                        if move_line.quantity:
                            ...
                            try:
                                move_line.write({"quantity": move_line.qty_done})
                            except UserError:
                                continue
    ...
    def _create_split_backorder(self, default=None):
        self.ensure_one()
        picking = self.env["stock.picking"].browse(self.env.context.get("active_ids", []))
            ...
            dict(
                {
                    ...
                    "shopify_config_id": picking.shopify_config_id.id,
                    "shopify_order_id": picking.shopify_order_id,
                },
                **(default or {})
            )
        )
        self.message_post(body=_('The backorder ' + backorder_picking._get_html_link() + ' has been created.'))
