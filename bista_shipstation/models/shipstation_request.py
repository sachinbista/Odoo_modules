...
class ShipStationRequest():
    ...
    def _make_api_request(self, endpoint, request_type='get', data=None, timeout=False):
        ...
        # access_url='https://ssapi.shipstation.com/orders/19783390'
        # request_type ='get'
        try:
            self.debug_logger("%s\n%s\n%s" % (access_url, request_type, data if data else None), 'shipstation_request_%s' % endpoint)
            ...
            if request_type == 'get':
                ...
                response = requests.put(access_url, auth=(self.api_key, self.api_secret), data=json_data, headers=headers)
            self.debug_logger("%s\n%s" % (response.status_code, response.text), 'shipstation_response_%s' % endpoint)
            if response.status_code == 400:
                error_message = response.text
                raise UserError(_('ShipStation returned an error: ') + '%d\nError message: %s' % (response.status_code, error_message))
    ...
    def send_shipping(self, carrier, recipient, shipper, picking, is_return=False):
        ...
        customer_note = picking.sale_id.name if picking.sale_id else '' + (' - %s' % picking.sale_id.client_order_ref if picking.sale_id.client_order_ref else '')
        carrier_type = picking.sale_id.ss_quotation_carrier or 'stamps_com'
        service_type = picking.sale_id.ss_quotation_service or carrier.shipstation_default_service_id.name or 'usps_priority_mail'
        # service_type2 = picking.sale_id.ss_quotation_service or carrier.shipstation_default_service_id.name or 'No Shipping Set'
        service_type2 = ''
        # for line in picking.sale_id:
        #     if line.product_id.detailed_type == 'service':
        #         service_type2 = line.name
        if  picking.sale_id:
        ...
        data = {
            ...
            # "shippingAmount": 1111,
            "internalNotes": None,
            "carrierCode": picking.partner_id.carrier_id.code if picking.partner_id.carrier_id else 'fedex',
            "serviceCode": picking.partner_id.service_id.code if picking.partner_id.service_id  else "fedex_home_delivery",
            "packageCode": "package",
            "confirmation": "delivery",
            ...
            "advancedOptions": {
                ...
                "customField2": picking.sale_id.customer_po

            },
        }
        # sale_order_obj = picking.env['sale.order']
        # sale_id = picking.sale_id

        # if picking.origin and not sale_id:
        #     sale_id = sale_order_obj.search([('name', '=', picking.origin)])
        # if sale_id and not sale_id.delivery_pay:
        #     shipping_cost = 0
        #     for line in sale_id.order_line:
        #         if line.is_delivery:={}
        #             shipping_cost += line.price_subtotal
        #     data['shippingAmount'] = shipping_cost
        #     sale_id.delivery_pay = True
