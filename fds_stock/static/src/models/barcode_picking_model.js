...
export default class BarcodePickingMoveBoxBalletModel extends BarcodeModel {
    constructor(params) {
        super(...arguments);
    ...
    _isOutGoing() {
        return this.picking_type_code === 'outgoing';
    }

    setData(data) {
        super.setData(...arguments);
        // Manage extra information for locations
        this.picking_type_code = data.data.picking_type_code;
        if (this._isOutGoing()) {
            this.lineModel = 'fds.move_box_and_pallet';
        }
    ...
    _createLinesState() {
        ...
        if (picking.picking_type_code === 'outgoing') {
            ...
            //         "id": 5,
            //         "barcode": false,
            //         "display_name": "Partners/Customers",
            //         "name": "Customers",
            //         "parent_path": "2/5/",
            //         "usage": "customer"
            //     },
            ...
            //     "move_id": 169,
            //     "description_bom_line": false,
            //     "virtual_id": 1
            // }
            for (const id of picking.move_line_box_pallet_ids) {
                const smlData = this.cache.getRecord('fds.move_box_and_pallet', id);
                // mapping fds.move_box_and_pallet to stock.move.line
                smlData.location_id = smlData.line_location_id
                smlData.location_dest_id = smlData.line_location_dest_id
                smlData.qty_done = smlData.qty_done
                smlData.display_name =  smlData.line_display_name
                smlData.reserved_uom_qty = smlData.line_product_uom_qty
                smlData.box_and_pallet = smlData.line_box_and_pallet
                smlData.loose = smlData.line_loose
                // TODO: set value
                smlData.owner_id = false
                smlData.lot_id = false
                smlData.lot_name = false
                smlData.package_id = false
                smlData.result_package_id = false
