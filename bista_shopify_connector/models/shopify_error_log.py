...
class ShopifyErrorLog(models.Model):
    ...
    def create_update_log(self, shop_error_log_id=False, shopify_config_id=False, operation_type='', shopify_log_line_dict=None):
        ...
        if not shopify_log_id:
            ...
            shopify_log_id = self.sudo().create(log_vals)
        else:
            shopify_log_id.sudo().write({'shop_error_log_line_ids': log_line_list})
    ...
    def create(self, vals):
        for val in vals:
            ...
            if rtn:
                extra_log = self.sudo().search([('id', '!=', rtn.id)]).filtered(
                    lambda l: not l.shop_error_log_line_ids)
                if extra_log:
                    extra_log.sudo().unlink()
        return rtn
...
class ShopifyErrorLogLine(models.Model):
    ...
    queue_job_line_id = fields.Many2one(
        "shopify.queue.job.line", string="Queue Line")