...
class ShopifyErrorLog(models.Model):
    ...
    def create_update_log(self, shop_error_log_id=False, shopify_config_id=False, operation_type='', shopify_log_line_dict=None):
        ...
        shopify_log_id = self.env.context.get('log_id', False) if not shop_error_log_id else shop_error_log_id
        shopify_log_line_dict = shopify_log_line_dict or self.env.context.get('shopify_log_line_dict', {})
        if shopify_log_line_dict:
            log_line_list.extend(self.prepare_create_line_vals(shopify_log_line_dict, 'error'))
        if not shopify_log_id:
            log_vals = {'shopify_config_id': shopify_config_id and shopify_config_id.id or False, 'operation_type': operation_type}
            if log_line_list:
    ...
    def create(self, vals):
        for val in vals:
            ...
            rtn = super().create(val)
            # for removed extra log
            # extra_log = self.search([('id', '!=', rtn.id)]).filtered(lambda l: not l.shop_error_log_line_ids)
            # if extra_log:
            #     extra_log.unlink()
        return rtn
...
class ShopifyErrorLogLine(models.Model):
    ...
    shop_error_log_id = fields.Many2one("shopify.error.log", "Shopify Error Log")
    ...
    queue_job_line_id = fields.Many2one("shopify.queue.job.line", string="Queue Line")
