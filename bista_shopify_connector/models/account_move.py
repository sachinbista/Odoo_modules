...
class AccountMove(models.Model):
    ...
    def fetch_all_shopify_orders(self, shopify_config):
        """return: shopify orders list
        TODO: fetch refund based on since_id - check with Ashvin """
        try:
            shopify_refunds_list = []
            page_info = False
            while 1:
                last_refund_import_date, parameter_id = shopify_config.get_update_value_from_config(
                    operation='read', field='last_refund_import_date', shopify_config_id=shopify_config,
                    field_value='')
                if last_refund_import_date:
                    if page_info:
                        page_wise_refund_list = shopify.Order.find(
                            limit=250, page_info=page_info)
                    else:
                        page_wise_refund_list = shopify.Order.find(
                            updated_at_min=last_refund_import_date or fields.Datetime.now(),
    ...
    def shopify_import_refund_orders(self, shopify_config):
        ...
        try:
            move_obj = self_cr.env['account.move']
            shopify_config.check_connection()
            shopify_order_list = self_cr.fetch_all_shopify_orders(shopify_config)
            if shopify_order_list:
                ...
                for shopify_orders in tools.split_every(250, shopify_order_list):
                    for order in shopify_orders:
                        ...
                        job_descr = _("Import Refund from shopify to Odoo: %s") % (
                                    name and name.strip())
                        log_line_vals.update({
                            'name': job_descr,
                            'id_shopify': f"Refund: {order_dict.get('id', '') or ''}",
                        ...
                        move_obj.with_company(shopify_config.default_company_id).with_delay(
                            description=job_descr, max_retries=5,
                            eta=eta).create_update_shopify_refund(order_dict,
                                                                  shopify_config,
    ...
    def create_update_shopify_refund(self, order_data, shopify_config,
        ...
        for refunds in get_refunds:
            ...
            if not order:
                error_message = "Facing a problems while importing refund.\n" \
                                "shopify order id!: %s not found in Odoo" % (order_id or '')
                log_line_id and log_line_id.write({
                    'state': 'error', 'message': error_message})
                raise ValidationError(_(error_message))
            shopify_transaction_id_list = self.env['account.move'].search(
                [('sale_order_id', '=', order.id), ('move_type', 'in', ['out_invoice', 'out_refund'])]).mapped(
                'shopify_transaction_id')
            if str(refund_dict.get('id')) in shopify_transaction_id_list:
                continue
            fulfillment_status = order_data.get('fulfillment_status')
            restock = refund_dict.get('restock')
            if not restock and fulfillment_status != 'fulfilled':
                #    - If product not restock	Flow 2: Create payment of customer - send money
                shopify_transactions = shopify.Transaction().find(order_id=str(order.shopify_order_id))
                for transaction in shopify_transactions:
                    transaction_dict = transaction.to_dict()
                    order.create_shopify_order_payment(transaction_dict, 'outbound')
                log_line_id and log_line_id.write(
                    {'state': 'success'})
                # if len(refund_dict.get('refund_line_items')) != 0:
                #     return True
            ...
            if customer:
                ...
                if not partner:
                    partner_env.shopify_import_customer_by_ids(
                        shopify_config, shopify_customer_by_ids=cust_id)
                    partner = partner_env.search(
                        [('shopify_customer_id', '=', cust_id)], limit=1)
            ...
            for shipping_line_data in order_data.get('shipping_lines'):
                ...
                if str(order_data.get('total_tax')) not in ['0.0', '0.00']:
                    for tax_line in shipping_line_data.get('tax_lines'):
                        ship_tax_price += round(float(
                            tax_line.get('price', 0.0)), 2)
                        rate = float(
                            tax_line.get('rate', 0.0))
                        tax_calc = rate * 100
            ...
            if not trans:
                error_message = "Facing a problems while importing refund.\n" \
                                "shopify order id!: %s transactions not found in Odoo" % (
                                            order_id or '')
                log_line_id and log_line_id.write({'state': 'error',
            ...
            for transaction in trans:
                ...
                if str(refund_transaction_data.get('id')) not in shopify_transaction_id_list:
                    transaction_list.append(str(refund_transaction_data.get(
                        'id')))
                    if status == 'success':
            ...
            if refund_dict.get('refund_line_items') and not order_adj_des:
                ...
                for refund_lines in refund_dict.get('refund_line_items'):
                    ...
                    inv_line_vals = (
                        {'product_id': prd_id, 'name': description,
                         ...
                         'quantity': qty,
                         'tax_ids': tax_ids and [(6, 0, tax_ids)] or [
                             (6, 0, [])],
                         'account_id': unearned_account_id.id if unearned_account_id else refund_journal_id.default_account_id.id,
                ...
                if line_lst and not product_missing:
                    ...
                    local_datetime = shopify_config.convert_shopify_datetime_to_utc(created_order_at)
                    vals = {
                        'move_type': 'out_refund',
                        'ref': 'Reversal of: %s' % order.name,
                        'invoice_origin': order_name,
                        'invoice_date': str(local_datetime),
                        'partner_id': partner_id or cust.id,  # billing_id and billing_id.id or partner_id,
                        'shopify_order_id': order_id,
                        'shopify_transaction_id': refund_id,
                        'currency_id': tcurrency_id.id if tcurrency_id else shopify_config.default_company_id.currency_id.id,
                        'invoice_line_ids': line_lst,
                        'narration': note,
                        'company_id': company_id,
                        'shopify_config_id': shopify_config.id,
                        'journal_id': refund_journal_id.id,
                        'partner_shipping_id': partner_id,  # shipping_id and shipping_id.id or partner_id,
                            ...
                            for rinv in exist_move:
                                ...
                                move_lines = payment_id.move_id.invoice_line_ids.filtered(
                                    lambda line: line.account_type in (
                                        'liability_payable',
                                        'asset_receivable') and not line.reconciled)
                                for line in move_lines:
                                    rinv.js_assign_outstanding_line(line.id)
                    log_line_id and log_line_id.write({'state': 'success'})
            else:
                ...
                for transaction in trans:
                    ...
                    for cr in credit_notes:
                        if cr.invoice_line_ids.filtered(
                                lambda l: l.shopify_transaction_id == str(transaction_id)):
                            te = cr.id
                            break
                    if credit_note:
                        log_line_id and log_line_id.write({'state': 'success'})
                ...
                if line_list:
                    try:
                        credit_note = self.with_context(custom_context).create(vals)
                        if shopify_config.is_refund_auto_paid:
        ...
        parameter_id = self.env['ir.config_parameter'].search([('key', '=', key_name)])
        shopify_config.get_update_value_from_config(
            operation='write', field='last_refund_import_date', shopify_config_id=shopify_config,
            field_value=str(datetime.now().strftime('%Y/%m/%d %H:%M:%S')), parameter_id=parameter_id)
        return True
    ...
    def action_open_refund_wizard(self):
        ...
        if not self.shopify_order_id:
            raise UserError(_("You cannot create refund in shopify as this invoice is not for shopify order."))
        if self.shopify_transaction_id:
            raise UserError(_("Refund already created in shopify with ID %s." % self.shopify_transaction_id))
        shipping_amount = 0.0
        order_shipping_amount = 0.0
        shopify_shipping_product_id = self.shopify_config_id.shipping_product_id
        shipping_line_id = self.invoice_line_ids.filtered(lambda r: r.product_id.id == shopify_shipping_product_id.id)
        shipping_so_line_id = shipping_line_id.sale_line_ids.filtered(
                lambda l:l.is_delivery and l.shopify_shipping_line)
        if shipping_so_line_id:
            shipping_amount = shipping_line_id and shipping_line_id.price_subtotal or 0.0
        refund_lines = self.invoice_line_ids.filtered(lambda r: r.product_id and r.product_id.type != 'service')
        line_ids = [(0, 0, {'product_id': line.product_id.id,
                            'quantity': line.quantity,
                            'shopify_line_id': line.sale_line_ids.shopify_line_id,
                            }) for line in refund_lines]
        if shipping_line_id:
            order_shipping_amount = shipping_line_id.sale_line_ids and shipping_line_id.sale_line_ids[0].price_subtotal or 0.0
        context = dict(self._context)

        amount_total = self.amount_total
        if self.sale_order_id:
            non_shopify_ship = (self.sale_order_id.order_line.filtered(
                lambda l:l.is_delivery and not l.shopify_shipping_line))
            if non_shopify_ship:
                invoice_line = self.invoice_line_ids.filtered(
                    lambda l: non_shopify_ship.id in l.sale_line_ids.ids)
                if invoice_line:
                    amount_total = (amount_total -
                                    sum(invoice_line.mapped('price_subtotal')))
        context.update({'default_shipping_refund_amount': shipping_amount,
                        'default_total_refund_amount': amount_total,
