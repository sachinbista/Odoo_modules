    ...
    def shopify_import_refund_orders(self, shopify_config):
        ...
        if shopify_order_list:
            ...
            for shopify_orders in tools.split_every(250, shopify_order_list):
                shop_queue_id = shopify_config.action_create_queue('import_refund')
            ...
            if str(order_data.get('total_tax')) not in ['0.0', '0.00']:
                for tax_line in shipping_line_data.get(
                    ...
                    ship_tax_price += round(float(
                        tax_line.attributes.get('price', 0.0)), 2)
                    rate = float(
                        tax_line.attributes.get('rate', 0.0))
                    tax_calc = rate * 100
        ...
        if refund_dict.get('refund_line_items') and not order_adj_des:
                ...
                inv_line_vals = (
                    {'product_id': prd_id, 'name': description,
                     ...
                     'tax_ids': tax_ids and [(6, 0, tax_ids)] or [
                         (6, 0, [])],
                     'account_id': unearned_account_id and unearned_account_id.id or refund_journal_id.loss_account_id.id,
                     'analytic_account_id': shopify_config.analytic_account_id.id,
            ...
            for transaction in trans:
                ...
                if shopify_config.is_refund_auto_paid:
                    ...
                    order.create_shopify_order_payment(refund_transaction_data,'outbound')
                    ...
                    error_log_env.create_update_log(shop_error_log_id=shop_error_log_id,
                                                    shopify_log_line_dict={'error': [
                                                        {'error_message': error_message,
                                                         'queue_job_line_id': queue_line_id and queue_line_id.id or False}]})
                    queue_line_id and queue_line_id.write({'state': 'failed'})
        shopify_config.last_refund_import_date = fields.Datetime.now()
        return True
...
class AccountPayment(models.Model):
    ...
    def create(self, vals_list):
        ...
        payment_gateway_obj = self.env['shopify.payment.gateway']
        for vals in vals_list:
            ref_value = vals.get('ref')
            if ref_value:
                move_id = account_move_obj.search([('name', 'like', ref_value)], limit=1)
                if move_id:
                    vals['sale_order_id'] = move_id.sale_order_id.id
            shopify_gateway = vals.get('shopify_payment_gateway_id')
            gateway_rec= payment_gateway_obj.search([('code','=',shopify_gateway)])
            if gateway_rec:
                vals['journal_id'] = gateway_rec.journal_id.id
        return super(AccountPayment, self).create(vals_list)

class AccountMoveLine(models.Model):
    _inherit = 'account.move.line'

    shopify_gateway = fields.Char(string='Shopify Gateway',compute='_get_shopify_getway_custom')

    @api.depends()
    def _get_shopify_getway_custom(self):
        for rec in self:
            if rec.payment_id and rec.payment_id.shopify_gateway:
                rec.shopify_gateway = rec.payment_id.shopify_gateway or ''
            else:
                rec.shopify_gateway = ''
