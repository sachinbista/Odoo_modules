...
class ShopifyPayout(models.Model):
    ...
    def shopify_import_payouts(self,shopify_config, start_date, end_date):
        ...
        try:
            payouts = shopify.Payouts().find(status="paid", date_min=start_date, date_max=end_date, limit=250)
        except Exception as error:
            error_message = "Something is wrong while import the payout records : {0}".format(error)
    ...
    def create_payout_lines(self, payout_data):
        ...
        transactions = shopify.Transactions().find(payout_id=self.payout_ref_id, limit=250)

        for transaction in transactions:
            transaction_data = transaction.to_dict()
            transaction_vals = self.prepare_transaction_vals(transaction_data, self.shopify_config_id)
    ...
    def generate_bank_statement(self):
        ...
        bank_statement_id = bank_statement_obj.search([('shopify_payout_ref', '=', self.payout_ref_id)], limit=1)
        if bank_statement_id:
            self.write({'statement_id': bank_statement_id.id})
            return True

        name = '{0}_{1}'.format(self.shopify_config_id.name, self.payout_ref_id)
        vals = {
        ...
        _logger.info("Bank Statement Generated for Shopify Payout : %s.", self.payout_ref_id)
    ...
    def create_bank_statement_lines_for_payout(self, bank_statement_id, regenerate=False):
        ...
        for transaction in transaction_ids:
            ...
            domain, invoice, shop_error_log_id = self.check_for_invoice_refund(transaction)
            if domain:
    ...
    def check_for_invoice_refund(self, transaction):
        ...
        if transaction.transaction_type == 'charge':
            ...
            if not invoice_ids:
                ...
                shop_error_log_id = error_log_env.create_update_log(shop_error_log_id=shop_error_log_id,
                                            shopify_log_line_dict={'error': [
                                                {'error_message': error_message,
                                                 'queue_job_line_id': queue_line_id and queue_line_id.id or False}]})
                queue_line_id and queue_line_id.write({'state': 'failed'})
