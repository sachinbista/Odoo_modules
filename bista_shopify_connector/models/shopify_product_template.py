...
class ShopifyProductTemplate(models.Model):
    ...
    last_updated_date = fields.Datetime(string='Last Updated Date',
                                        readonly=True, copy=False)
    shopify_handle = fields.Char("Handle", copy=False, tracking=True)
            ...
            while 1:
                if shopify_config.last_product_import_date:
                    ...
                    if page_info:
                        ...
                        page_wise_product_list = shopify.Product().find(limit=10)
    ...
    def shopify_import_product(self, shopify_config):
        ...
        if shopify_product_list:
            for shopify_products in tools.split_every(10, shopify_product_list):
        ...
        try:
            ...
            for attrib in result.get('options'):
                ...
                for attrib_vals in attrib_values:
                    ...
                    if not attrib_value:
                        attrib_value = product_attribute_value_obj.sudo().create(
                            {'attribute_id': attribute.id,
                             'name': attrib_vals,
                             })
        ...
        for variation in result.get('variants'):
            ...
            for variation_attribute in variation_attributes:
                ...
                if not product_attribute:
                    product_attribute = product_attribute_obj.sudo().create(
                        {'name': attribute_name})
                if product_attribute:
                    ...
                    if not product_attribute_value:
                        product_attribute_value = product_attribt_val_obj.sudo().create(
                            {'attribute_id': product_attribute.id,
                             'name': attribute_val,
                             })
            ...
            if attribute_value_ids:
                ...
                attribute_ids = pt_atribt_ids.filtered(
                    lambda a:  a.product_attribute_value_id.id in attribute_value_ids)
                odoo_product = product_id.filtered(
                    lambda a: a.product_template_attribute_value_ids == attribute_ids)
                if odoo_product:
                    odoo_product.write({'default_code': sku})
                    # if odoo_product and sku:
                    #     odoo_product.write({'title':f"{[sku]} {result.get('title', '')}".replace("'", "")})
                    # elif odoo_product and barcode:
                    #     odoo_product.write({'title':f"{[barcode]} {result.get('title', '')}".replace("'", "")})
                    # else:
                    #     odoo_product and odoo_product.write({'title':f"{result.get('title', '')}"})
                if price:
                    odoo_product and odoo_product.write({'list_price': price})
                if weight:
                    odoo_product and odoo_product.write({'weight': weight})
    ...
    def check_product_exist(self, product_dict, shopify_config_id):
        product_create_update = False
        shopify_template = self.search(
            [('shopify_prod_tmpl_id', '=', str(product_dict.get('id'))),
             ('shopify_config_id', '=', shopify_config_id.id)])
        if shopify_template:
            odoo_product_variant_len = len(
                shopify_template.product_tmpl_id.product_variant_ids.ids)
            shopify_product_variant_len = len(product_dict.get('variants'))
            if odoo_product_variant_len != shopify_product_variant_len:
                product_create_update = True
            return product_create_update
        else:
            return True
    ...
    def create_update_shopify_product(self, product_dict, shopify_config_id):
        ...
        shopify_product_obj = self.env['shopify.product.product'].sudo()
        product_template_obj = self.env['product.template'].sudo()
        product_category_obj = self.env['product.category'].sudo()
        partner_obj = self.env['res.partner'].sudo()
        product_result = self.check_product_exist(
            product_dict, shopify_config_id)
        if product_result:
            try:
                is_service_product = False
                tmpl_id = False
                    ...
                    if not odoo_product and not shopify_variant:
                        ...
                        # for product without variants
                            ...
                            if not odoo_product and sync_product == 'sku' and not sku or sync_product == 'barcode' and not barcode:
                                error_message = "Odoo product not found for sku: %s or barcode: %s" % (
                                    sku, barcode)
                                error_log_env.sudo().create_update_log(
                                    shop_error_log_id=shop_error_log_id,
                                    shopify_log_line_dict={
                            ...
                            if not tmpl_id:
                                ...
                                # create product template
                                if shopify_config_id.is_create_product:
                                    tmpl_id = product_template_obj.create(
                                        temp_vals)
                                else:
                                    error_message = "Odoo product not created for %s.\n" \
                                                    "if you want auto created it. enable it from shopify configuration" % (
            ...
            template_title = product_dict.get('title', '')
            error_message = f"Product {template_title} Already Exists!"
            error_log_env.sudo().create_update_log(
                shop_error_log_id=shop_error_log_id,
                shopify_log_line_dict={
                    'error': [
                ...
                if shopify_config.last_stock_import_date:
                    if page_info:
                        ...
                        page_wise_inventory_list = shopify.InventoryLevel().find(
                            updated_at_min=shopify_config.last_stock_import_date, location_ids=','.join(shopify_location_ids))
                else:
                    if page_info:
                        page_wise_inventory_list = shopify.InventoryLevel().find(page_info=page_info)
                    else:
    ...
    def shopify_import_stock(self, shopify_config):
        ...
        if shopify_product_template_ids:
            product = shopify_product_template_ids.filtered(
                lambda sp: sp.product_tmpl_id.detailed_type in ['consu', 'service'])
            if product:
                pass
            # shopify_config.check_connection()
            if shopify_config.state == 'success':
                location_ids = self.env['stock.location'].sudo().search(
                    [('shopify_config_id', '=', shopify_config.id),
                     ('shopify_location_id', '!=', False)])
                shopify_location_ids = location_ids.mapped(
                    'shopify_location_id')
                ...
                log_message = f"Before importing stock please make sure connection should be in success state, still now is in {shopify_config.state} state."
                error_log_env.sudo().create_update_log(
                    shop_error_log_id=shopify_log_id,
                    shopify_log_line_dict={
                        'error': [{'error_message': log_message}]})
