...
class ClientPerformance(models.AbstractModel):
    _name = "client.performance"
    ...
    def get_crm_details(self, date_from, date_to, customer_ids, stage):
        ...
        sql = ("""
            SELECT COUNT(cl.id) as count, SUM(cl.expected_revenue) as expected_revenue, SUM(cl.day_close) as day_close FROM crm_lead cl 
    ...
    def _get_stagewise_crm_lines(self, calculate_avg_days, opportunity_no, opportunity_amount, month_start_date,
        ...
        mnth_opportunities_count, mnth_amount, day_close = self.get_crm_details(month_start_date, date_to, customer_ids,
                                                                                stage)
    ...
    def get_sale_amount(self, date_from, date_to, customer_ids, sale_type='sale', check_delivery=None):
        if date_from:
            date_from += ' 00:00:00'
        if date_to:
            date_to += ' 23:59:59'
        ...
        if check_delivery:
            ...
            select 
                ...
                ,sum(case when pt.list_price = 0.0 and pt.type != 'service' then (sol.price_unit*sm.product_uom_qty) 
                when pt.type = 'service' and pt.list_price = 0.0  then (sol.price_unit * sol.qty_delivered)
                when pt.type = 'service' and pt.list_price != 0.0 then (pt.list_price*sol.qty_delivered)
                else (pt.list_price* sm.product_uom_qty) end) as sale_price
                ,sum(case when sol.discount = 0.0 and pt.type != 'service' then (sol.price_unit*sm.product_uom_qty)
                when  pt.type = 'service' then sol.price_subtotal
            ...
            where sp.date_done >= '%s' and sp.date_done <= '%s' 
                and sp.state = 'done' and spt.code='outgoing' and sm.product_id = sol.product_id and so.partner_id in %s
                and pt.exclude_from_report = False and pc.sale_type = '%s' 
            """) % (date_from, date_to, customer_ids, sale_type)
        else:
            sql = ("""
            select 
            ...
            where ai.state in ('open','paid') and 
                so.date_order >= '%s' and so.date_order <= '%s' and so.partner_id in %s and 
                pt.exclude_from_report = False and pc.sale_type = '%s' 
            """) % (date_from, date_to, customer_ids, sale_type)
        ...
        return retail_sale, invoice_sale, gross_margin

    def get_sale_amount_of_pos(self, date_from, date_to, customer_ids, sale_type='sale', check_delivery=None):
        if date_from:
            date_from += ' 00:00:00'
        if date_to:
            date_to += ' 23:59:59'
        ...
        select 
            sum(sm.product_uom_qty)
            ,sum(case when pt.list_price = 0.0 then (pol.price_unit*sm.product_uom_qty) else (pt.list_price* sm.product_uom_qty) end) as sale_price_1
            ,sum(pol.price_subtotal) as sale_price
            ,sum(case when pol.discount = 0.0 then (pol.price_unit*sm.product_uom_qty) else ((pol.price_unit*sm.product_uom_qty)* (pol.discount))/100.0 end) as invoice_price_1
            ,sum(pol.price_subtotal) as invoice_price
            ,sum(case when pp.std_price= 0.0 then 0 else (pp.std_price * sm.product_uom_qty) end) as cost_price
            ,sum(case when pol.discount = 0.0 then (pol.price_unit*sm.product_uom_qty) else ((pol.price_unit*sm.product_uom_qty)* (pol.discount))/100.0 end - case when pp.std_price= 0.0 then 0 else (pp.std_price * sm.product_uom_qty) end)  as gross_margin
        from pos_order_line pol
            ...
            inner join product_category pc on (pc.id = pt.categ_id) 
        where sp.date_done >= '%s' and sp.date_done <= '%s'
            and sp.state = 'done' and spt.code in ('outgoing') and sm.product_id = pol.product_id
            and po.partner_id in %s and pt.exclude_from_report = False and pc.sale_type = '%s' 
        """) % (date_from, date_to, customer_ids, sale_type)
    ...
    def get_so_return_amount_of_pos(self, customer_ids, date_from, date_to):
        if date_from:
            date_from += ' 00:00:00'
        if date_to:
            date_to += ' 23:59:59'
        ...
        select 
            sum(sm.product_uom_qty)
            ,sum(case when pol.discount = 0.0 then (pol.price_unit*sm.product_uom_qty) else ((pol.price_unit*sm.product_uom_qty)* (pol.discount))/100.0 end) as invoice_price 
        from pos_order_line pol
            inner join pos_order po on (po.id = pol.order_id)
            inner join stock_picking sp on (sp.pos_order_id = po.id) or (sp.pos_session_id = po.session_id)
            inner join stock_picking_type spt on (spt.id = sp.picking_type_id)
            ...
            inner join product_category pc on (pc.id = pt.categ_id) 
        where sp.date_done >= '%s' and sp.date_done <= '%s'
            and sp.state = 'done' and spt.code in ('incoming') and sm.product_id = pol.product_id
            and po.partner_id in %s and pt.exclude_from_report = False and pc.sale_type = '%s' 
        """) % (date_from, date_to, customer_ids, sale_type)
        ...
        return -abs(mnth_so_return_amount)

    def get_so_return_amount(self, customer_ids, date_from, date_to):
        if date_from:
            date_from += ' 00:00:00'
        if date_to:
            date_to += ' 23:59:59'
        ...
        sql = ("""
            select 
                sum(sm.product_uom_qty)
                ,sum(case when sol.discount = 0.0 then (sol.price_unit*sm.product_uom_qty) else ((sol.price_unit*sm.product_uom_qty)* (sol.discount))/100.0 end) as invoice_price  
            from sale_order_line sol
                inner join sale_order so on (so.id = sol.order_id)
                inner join stock_picking sp on (sp.group_id = so.procurement_group_id)
                inner join stock_picking_type spt on (spt.id = sp.picking_type_id)
                ...
                inner join product_category pc on (pc.id = pt.categ_id) 
            where sp.date_done >= '%s' and sp.date_done <= '%s' 
                and sp.state = 'done' and spt.code='incoming' and sm.product_id = sol.product_id and so.partner_id in %s
                and pt.exclude_from_report = False and pc.sale_type = '%s' 
            """) % (date_from, date_to, customer_ids, sale_type)
    ...
    def get_sale_count(self, date_from, date_to, customer_ids, sale_type='sale'):
        if date_from:
            date_from += ' 00:00:00'
        if date_to:
            date_to += ' 23:59:59'
        ...
        select 
            SUM(CASE WHEN spt.code in ('outgoing') THEN (sm.product_uom_qty)
                    ...
                    when pt.type = 'service' then sol.qty_delivered
                    ELSE 0
               END) as net_qty_1,
        ...
        from sale_order_line sol
            ...
            inner join product_category pc on (pc.id = pt.categ_id) 
        where sp.date_done >= '%s' and sp.date_done <= '%s' 
            and sp.state = 'done' and spt.code in ('outgoing', 'incoming') and sm.product_id = sol.product_id and so.partner_id in %s
            and pt.exclude_from_report = False and pc.sale_type = '%s' 
        """) % (date_from, date_to, customer_ids, sale_type)
        ...
        return sale_count, product_count

    def get_sale_count_of_pos(self, date_from, date_to, customer_ids, sale_type='sale'):
        if date_from:
            date_from += ' 00:00:00'
        if date_to:
            date_to += ' 23:59:59'
    ...
    def get_transaction_lines(self, net_sale_list, month_start_date, date_to,
        ...
        if customer_ids:
            ...
            m_sale_count, m_item_count = self.get_sale_count(str(month_start_date), str(date_to), customer_ids)
            m_sale_count_pos, m_item_count_pos = self.get_sale_count_of_pos(str(month_start_date), str(date_to),
                                                                            customer_ids)
            ...
            ly_m_sale_count, ly_m_item_count = self.get_sale_count(str(last_yr_month_start_date), str(last_yr_date_to),
                                                                   customer_ids)
            ly_m_sale_count_pos, ly_m_item_count_pos = self.get_sale_count_of_pos(str(last_yr_month_start_date),
                                                                                  str(last_yr_date_to), customer_ids)
            ...
            y_sale_count, y_item_count = self.get_sale_count(str(fiscalyear_start_date), str(date_to), customer_ids)
            y_sale_count_pos, y_item_count_pos = self.get_sale_count_of_pos(str(fiscalyear_start_date), str(date_to),
                                                                            customer_ids)
            ...
            ly_y_sale_count, ly_y_item_count = self.get_sale_count(str(last_yr_fiscalyear_start_date),
                                                                   str(last_yr_date_to), customer_ids)
            ly_y_sale_count_pos, ly_y_item_count_pos = self.get_sale_count_of_pos(str(last_yr_fiscalyear_start_date),
                                                                                  str(last_yr_date_to), customer_ids)
    ...
    def get_repair_service_lines(self, row_name, sale_type, month_start_date, date_to,
        ...
        if customer_ids:
            ...
            mnth_retail_amount, mnth_invoice_amount, mnth_gross_amount = self.get_sale_amount(str(month_start_date),
                                                                                              date_to, customer_ids,
                                                                                              sale_type)
            mnth_retail_amount_pos, mnth_invoice_amount_pos, mnth_gross_amount_pos = self.get_sale_amount_of_pos(
                str(month_start_date), date_to, customer_ids, sale_type)
            ...
            ly_mnth_retail_amount, ly_mnth_invoice_amount, ly_mnth_gross_amount = self.get_sale_amount(
                str(last_yr_month_start_date), str(last_yr_date_to), customer_ids, sale_type)
            ly_mnth_retail_amount_pos, ly_mnth_invoice_amount_pos, ly_mnth_gross_amount_pos = self.get_sale_amount_of_pos(
                str(last_yr_month_start_date), str(last_yr_date_to), customer_ids, sale_type)
            ...
            yr_retail_amount, yr_invoice_amount, yr_gross_amount = self.get_sale_amount(str(fiscalyear_start_date),
                                                                                        date_to, customer_ids,
                                                                                        sale_type)
            yr_retail_amount_pos, yr_invoice_amount_pos, yr_gross_amount_pos = self.get_sale_amount_of_pos(
                str(fiscalyear_start_date), date_to, customer_ids, sale_type)
            ...
            ly_yr_retail_amount, ly_yr_invoice_amount, ly_yr_gross_amount = self.get_sale_amount(
                str(last_yr_fiscalyear_start_date), str(last_yr_date_to), customer_ids, sale_type)
            ly_yr_retail_amount_pos, ly_yr_invoice_amount_pos, ly_yr_gross_amount_pos = self.get_sale_amount(
                str(last_yr_fiscalyear_start_date), str(last_yr_date_to), customer_ids, sale_type)
    ...
    def get_sale_lines(self, month_start_date, date_to, last_yr_month_start_date, last_yr_date_to,
        ...
        if customer_ids:
            ...
            mnth_retail_amount, mnth_invoice_amount, mnth_gross_amount = self.get_sale_amount(str(month_start_date),
                                                                                              ...
                                                                                              check_delivery=True)
            mnth_retail_amount_pos, mnth_invoice_amount_pos, mnth_gross_amount_pos = self.get_sale_amount_of_pos(
                str(month_start_date), date_to, customer_ids, check_delivery=True)
            mnth_retail_amount = mnth_retail_amount + mnth_retail_amount_pos
            mnth_invoice_amount = mnth_invoice_amount + mnth_invoice_amount_pos
            mnth_gross_amount = mnth_gross_amount + mnth_gross_amount_pos
            mnth_so_return_amount = self.get_so_return_amount(customer_ids, str(month_start_date), date_to)
            mnth_so_return_amount_pos = self.get_so_return_amount_of_pos(customer_ids, str(month_start_date), date_to)
            mnth_so_return_amount = mnth_so_return_amount + mnth_so_return_amount_pos

            ly_mnth_retail_amount, ly_mnth_invoice_amount, ly_mnth_gross_amount = self.get_sale_amount(
                str(last_yr_month_start_date), str(last_yr_date_to), customer_ids, check_delivery=True)
            ly_mnth_retail_amount_pos, ly_mnth_invoice_amount_pos, ly_mnth_gross_amount_pos = self.get_sale_amount_of_pos(
                str(last_yr_month_start_date), str(last_yr_date_to), customer_ids, check_delivery=True)
            ly_mnth_retail_amount = ly_mnth_retail_amount + ly_mnth_retail_amount_pos
            ly_mnth_invoice_amount = ly_mnth_invoice_amount + ly_mnth_invoice_amount_pos
            ...
            ly_mnth_so_return_amount = self.get_so_return_amount(customer_ids, str(last_yr_month_start_date),
                                                                 str(last_yr_date_to))
            ly_mnth_so_return_amount_pos = self.get_so_return_amount_of_pos(customer_ids, str(last_yr_month_start_date),
                                                                            str(last_yr_date_to))
            ...
            yr_retail_amount, yr_invoice_amount, yr_gross_amount = self.get_sale_amount(str(fiscalyear_start_date),
                                                                                        ...
                                                                                        check_delivery=True)
            yr_retail_amount_pos, yr_invoice_amount_pos, yr_gross_amount_pos = self.get_sale_amount_of_pos(
                str(fiscalyear_start_date), date_to, customer_ids, check_delivery=True)
            yr_retail_amount = yr_retail_amount + yr_retail_amount_pos
            yr_invoice_amount = yr_invoice_amount + yr_invoice_amount_pos
            yr_gross_amount = yr_gross_amount + yr_gross_amount_pos

            yr_so_return_amount = self.get_so_return_amount(customer_ids, str(fiscalyear_start_date), date_to)
            yr_so_return_amount_pos = self.get_so_return_amount_of_pos(customer_ids, str(fiscalyear_start_date),
                                                                       date_to)
            yr_so_return_amount = yr_so_return_amount + yr_so_return_amount_pos

            ly_yr_retail_amount, ly_yr_invoice_amount, ly_yr_gross_amount = self.get_sale_amount(
                str(last_yr_fiscalyear_start_date), str(last_yr_date_to), customer_ids, check_delivery=True)
            ly_yr_retail_amount_pos, ly_yr_invoice_amount_pos, ly_yr_gross_amount_pos = self.get_sale_amount_of_pos(
                str(last_yr_fiscalyear_start_date), str(last_yr_date_to), customer_ids, check_delivery=True)
            ly_yr_retail_amount = ly_yr_retail_amount + ly_yr_retail_amount_pos
            ly_yr_invoice_amount = ly_yr_invoice_amount + ly_yr_invoice_amount_pos
            ly_yr_gross_amount = ly_yr_gross_amount + ly_yr_gross_amount_pos

            ly_yr_so_return_amount = self.get_so_return_amount(customer_ids, str(last_yr_fiscalyear_start_date),
                                                               str(last_yr_date_to))
            ly_yr_so_return_amount_pos = self.get_so_return_amount_of_pos(customer_ids,
                                                                          str(last_yr_fiscalyear_start_date),
                                                                          str(last_yr_date_to))
    ...
    def get_crm_lines(self, month_start_date, date_to,
    ...
    def _get_lines(self, options, line_id=None):
        ...
        result_list = self.get_sale_lines(month_start_date, date_to, last_yr_month_start_date, last_yr_date_to,
                                          fiscalyear_start_date, last_yr_fiscalyear_start_date)
        ...
        line_id += 1
        result_list = self.get_transaction_lines(net_sale_list, month_start_date, date_to,
                                                 last_yr_month_start_date, last_yr_date_to, fiscalyear_start_date,
                                                 last_yr_fiscalyear_start_date)
        for values in result_list:
            list = self.update_symbols(values)
        ...
        line_id += 1
        result_list = self.get_repair_service_lines('Repairs', 'repair', month_start_date, date_to,
                                                    last_yr_month_start_date, last_yr_date_to, fiscalyear_start_date,
                                                    last_yr_fiscalyear_start_date)
        for values in result_list:
            list = self.update_symbols(values)
        ...
        line_id += 1
        result_list = self.get_repair_service_lines('Services', 'service', month_start_date, date_to,
                                                    last_yr_month_start_date, last_yr_date_to, fiscalyear_start_date,
                                                    last_yr_fiscalyear_start_date)
        for values in result_list:
            list = self.update_symbols(values)
        ...
        result_list = self.get_crm_lines(month_start_date, date_to,
                                         last_yr_month_start_date, last_yr_date_to, fiscalyear_start_date,
                                         last_yr_fiscalyear_start_date)
        for values in result_list:
            list = self.update_symbols(values)
    ...
    def _get_reports_buttons(self):
        buttons = super(ClientPerformance, self)._get_reports_buttons()
        return buttons

    def _get_default_summary(self, options):

        return 'Includes Open And Unpaid Invoices'

    def _get_report_manager(self, options):

        domain = [('report_name', '=', 'client.performance'), ('company_id', '=', self.env.user.company_id.id)]
        ...
        if not existing_manager:
            existing_manager = self.env['account.report.manager'].create({
                'report_name': 'client.performance',
