...
class PrintProductLabelSection(models.Model):
    ...
    def _get_price_value(self, label, pricelist=False, min_quantity=0) -> str:
        self.ensure_one()
        section = self
    ...
    def name_get(self):
        return [(rec.id, "%s%s" % (
            rec.type == 'text'
            and (rec.value and _("Text: \"%s\"") % rec.value or _('Blank'))
            or rec.type == 'price' and 'Price'
            or rec.type == 'promo_price' and 'Promo Price'
            or rec.type == 'multi_price' and 'Multiple prices (by qty)'
            or rec.type == 'product_attributes' and 'Product Attributes'
            or rec.type == 'image' and _('Image')
            or rec.type == 'field'
            and "%s %s" % (
