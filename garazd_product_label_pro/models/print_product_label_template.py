...
class PrintProductLabelTemplate(models.Model):
    ...
    def get_demo_product_label(self):
        ...
        pricelist = pricelist_id \
            and self.env['product.pricelist'].browse(pricelist_id) \
            or self.env.company.print_label_preview_pricelist_id

        sale_pricelist_id = self._context.get('sale_pricelist_id')
        sale_pricelist = sale_pricelist_id \
            and self.env['product.pricelist'].browse(sale_pricelist_id) \
            or self.env.company.print_label_preview_sale_pricelist_id
    ...
    def _compute_preview_html(self):
        demo_product = self.env.company.print_label_preview_product_id
        for template in self:
            template.preview_html = demo_product and template.get_preview_html() or ''
