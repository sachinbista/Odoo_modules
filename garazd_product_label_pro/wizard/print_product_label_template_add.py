...
class PrintProductLabelTemplateAdd(models.TransientModel):
    ...
    def _get_label_name(self):
        self.ensure_one()
        paperformat_name = \
            self.paper_format == 'custom' and 'Custom' or self.paper_format
        page_sizes = self.page_width and self.page_height \
            and f' {self.page_width}x{self.page_height} mm' or ''
        layout_name = (self.paper_format != 'custom' or self._is_multi_layout()) and \
            f' ({paperformat_name}{page_sizes}: ' \
            f'{self.cols * self.rows} pcs, {self.cols}x{self.rows})' or ''
